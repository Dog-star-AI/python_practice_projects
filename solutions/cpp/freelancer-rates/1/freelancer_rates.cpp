...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double rate = daily_rate(static_cast<double>(hourly_rate)) * 22.0;
    
    return std::ceil((rate - (rate * static_cast<double>(discount)/100.0)));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double after_discount = apply_discount(daily_rate(hourly_rate), discount);
    
    return std::floor(budget/after_discount);
}
