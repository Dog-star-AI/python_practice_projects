...
double interest_rate(double balance) {
    ...
    double rate = 0.0;
    if(balance < 0.0) rate = 3.213;
    else if(balance >= 0.0 && balance < 1000.0) rate = 0.5;
    else if(balance >= 1000.0 && balance < 5000.0) rate = 1.621;
    else if(balance >= 5000.0) rate = 2.475;
...
int years_until_desired_balance(double balance, double target_balance) {
    ...
    while(true){
        if(balance < target_balance){
            ...
            years += 1;
            continue;
        }else if(balance >= target_balance) return years;
    }
}
